...
class File{
    ...
    function error_log($error_type  , $type = "" , $extension = ""){
        ...
        print_r("<script>console.log('" . $text . "')</script>");
    }
    ...
    function zipDownload($obj , $path = ""){//압축해서 다운로드 
        ...
        if(gettype($obj) == "object"){ //오브젝트로 넘어올시 
            ...
            self::error_log("UPLOAD ERROR" , "object , array" , $obj ); //넘어오는 데이터 타입이 형식에 안맞을 때 
        }
    ...
    function zipUpload($obj , $path = ""){//압축해서 업로드 
        ...
        if($zip->open($zipname , ZipArchive::CREATE) == true){//알집파일 생성
            ...
            if($files['tmp_name'] == null && $files['tmp_name'] == ""){ //기존 파일 업로드 
            ...
            if($files['tmp_name'] != null && $files['tmp_name'] != ""){ //바로 업로드시 압축파일로 업로드
                ...
                foreach($files["tmp_name"] as $key => $data){
                    ...
                    if(file_exists($tmpName)){
                        $zip->addFile($tmpName , $file); //압축파일에 파일 넣기
                    }else{
                        self::error_log("NOT FILE");
                        return false;
                    }
